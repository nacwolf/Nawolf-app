...
router.get("/packaging", requireAuth, async (_req, res): Promise<void> => {
  try {
    ...
    console.error("[packaging] GET /packaging failed:", msg);
    res.status(500).json({ error: "Failed to load packaging items", detail: msg });
  }
});
...
router.post("/packaging", requireAuth, async (req, res): Promise<void> => {
  ...
  console.log("[packaging] POST body:", JSON.stringify({ nameEnglish, category, unit, unitCost }));
  ...
  try {
    ...
    const msg = err instanceof Error ? err.message : String(err);
    console.error("[packaging] POST /packaging failed:", msg);
    // Hint: if this says 'relation "packaging_items" does not exist',
    // run `pnpm --filter @workspace/db push` to create the table.
    res.status(500).json({ error: "Failed to create packaging item", detail: msg });
  }
});
...
router.get("/packaging/:id", requireAuth, async (req, res): Promise<void> => {
  ...
  try {
    ...
    const msg = err instanceof Error ? err.message : String(err);
    console.error("[packaging] GET /packaging/:id failed:", msg);
    res.status(500).json({ error: "Failed to load packaging item", detail: msg });
  }
});
...
router.patch("/packaging/:id", requireAuth, async (req, res): Promise<void> => {
  ...
  try {
    ...
    const msg = err instanceof Error ? err.message : String(err);
    console.error("[packaging] PATCH /packaging/:id failed:", msg);
    res.status(500).json({ error: "Failed to update packaging item", detail: msg });
  }
});
...
router.delete("/packaging/:id", requireAuth, async (req, res): Promise<void> => {
  ...
  try {
    ...
    const msg = err instanceof Error ? err.message : String(err);
    console.error("[packaging] DELETE /packaging/:id failed:", msg);
    res.status(500).json({ error: "Failed to delete packaging item", detail: msg });
  }
});
